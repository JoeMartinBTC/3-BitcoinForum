...
export function EventCard({ event, onUpdate }: EventCardProps) {
  ...
  return (
    <div ref={drag} style={{ opacity: isDragging ? 0.5 : 1 }} className="h-full">
      <Card className={`py-0.5 px-1 cursor-move hover:shadow-md transition-shadow w-full h-full overflow-hidden group ${template.color}`}>
        <div className="flex h-full items-center">
          <div className="flex flex-1 min-w-0">
            {event.title.length <= 25 && Icon && (
              <Icon className="h-2.5 w-2.5 shrink-0 mt-[1px] mr-1" />
            )}
            <span className={`font-medium text-left whitespace-nowrap overflow-hidden overflow-ellipsis ${
              event.title.length > 50 ? 'text-[0.6rem]' : 
              event.title.length > 30 ? 'text-[0.7rem]' : 
              'text-[0.8rem]'
