...
export function EventCard({ event, onUpdate }: EventCardProps) {
  ...
  const iconKey =
    template?.icon && template.icon in ICONS ? template.icon : "users";
  ...
  return (
    <TooltipProvider>
      ...
      >
        <div
          className={`cursor-move hover:shadow-md transition-shadow w-full h-full overflow-hidden group ${template.color}`}
        >
                    ...
                    >
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <Info
                            className={`inline-block ml-1 w-3 h-3 ${event.info ? "text-green-500" : "text-gray-400"}`}
                          />
                        </TooltipTrigger>
                        <TooltipContent>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {eventTypes.map((t) => (
                        <Button
                          key={t.id}
                          variant={
                            event.templateId === t.id ? "default" : "outline"
                          }
