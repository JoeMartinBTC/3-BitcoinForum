...
export function EventCard({ event, onUpdate }: EventCardProps) {
  ...
  return (
    <div ref={drag} style={{ opacity: isDragging ? 0.5 : 1 }} className="h-full">
      <Card className={`py-1 px-2 cursor-move hover:shadow-md transition-shadow w-full h-full overflow-hidden group`} style={{backgroundColor: event.color.startsWith('#') ? event.color : undefined}} className={event.color.startsWith('bg-') ? event.color : undefined}>
